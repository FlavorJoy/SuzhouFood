...
# -*- coding: utf-8 -*-
"""
README Renderer - 从CSV生成美观的README.md#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
def generate_stats_markdown(stats: Dict[str, Any], section_id: str = "data-statistics") -> List[str]:
    """生成统计信息的Markdown（使用带 id 的 HTML 标题以保证锚点一致）"""
    lines = []

    lines.append(f'<h2 id="{section_id}">📊 数据统计</h2>\n\n')
    lines.append("| 指标 | 数值 |\n")
    lines.append("| :--- | ---: |\n")
...
def generate_main_table(rows: List[Dict], section_id: str = "restaurant-list") -> List[str]:
    """生成主表格（带 id 的标题）"""
    lines = []
    lines.append(f'<h2 id="{section_id}">📋 餐厅列表</h2>\n\n')
...
def generate_detail_table(rows: List[Dict], section_id: str = "detailed-info") -> List[str]:
    """生成详细信息表格（折叠）并在前面添加带 id 的标题"""
    lines = []
    lines.append(f'<h2 id="{section_id}">📖 详细信息</h2>\n\n')
...
def generate_search_guide(section_id: str = "how-to-use") -> List[str]:
    """生成搜索和使用指南（带 id）"""
    lines = []
    lines.append(f'<h2 id="{section_id}">🔍 如何使用</h2>\n\n')
...
def generate_legend(section_id: str = "legend") -> List[str]:
    ...
    lines = []
    lines.append("\n---\n\n")
    lines.append(f'<h2 id="{section_id}">📌 图例说明</h2>\n\n')
    lines.append("| 图标 | 含义 |\n")
    lines.append("| :---: | --- |\n")
...
def generate_footer(section_id: str = "contribution") -> List[str]:
    ...
    lines = []
    lines.append("\n---\n\n")
    lines.append(f'<h2 id="{section_id}">📝 贡献指南</h2>\n\n')
...
def main():
    ...
    # 目录（使用与章节一致的 id）
    lines.append("## 📑 目录\n\n")
    lines.append("- [📊 数据统计](#data-statistics)\n")
    lines.append("- [📋 餐厅列表](#restaurant-list)\n")
    lines.append("- [📖 详细信息](#detailed-info)\n")
    lines.append("- [🔍 如何使用](#how-to-use)\n")
    lines.append("- [📌 图例说明](#legend)\n")
    lines.append("- [📝 贡献指南](#contribution)\n\n")

    lines.extend(generate_stats_markdown(stats))            # 默认 id "data-statistics"
    lines.extend(generate_main_table(rows))                 # 默认 id "restaurant-list"
    lines.extend(generate_detail_table(rows))               # 默认 id "detailed-info"
    lines.extend(generate_search_guide())                   # 默认 id "how-to-use"
    lines.extend(generate_legend())                         # 默认 id "legend"
    lines.extend(generate_footer())                         # 默认 id "contribution"
