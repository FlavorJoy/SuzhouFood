...
def get_top_rated(rows, limit=5):
    """获取评分最高的餐厅"""
    sorted_rows = sorted(rows, key=lambda r: parse_rating(r.get("rating", "")), reverse=True)
    return sorted_rows[:limit]


def get_queue_time_avg(rows):
    """计算平均排队时间"""
    times = []
    for r in rows:
        queue_time = (r.get("queue_time") or "").strip()
        if queue_time and '-' in queue_time:
            # 处理 "5-20分钟" 格式
            parts = queue_time.replace('分钟', '').split('-')
            if len(parts) == 2:
                try:
...
def generate_statistics(rows):
    """生成统计信息"""
    total = len(rows)
    avg_rating = sum(parse_rating(r.get("rating", "")) for r in rows) / total if total > 0 else 0
    ...
    # 高分餐厅
    top_rated = get_top_rated(rows)

    # 平均排队时间
    avg_queue = get_queue_time_avg(rows)
...
def generate_stats_markdown(stats):
    ...
    if stats['avg_queue'] > 0:
        lines.append(f"| ⏱️ 平均排队时间 | **{stats['avg_queue']:.0f}** 分钟 |\n")
    lines.append("\n")
...
def generate_main_table(rows):
    ...
    for r in rows:
        ...
        if "需要" in reservation or "是" in reservation:
            reservation = "✅ 需要"
        elif "否" in reservation or "不需" in reservation:
            reservation = "❌ 不需"
        else:
            reservation = "—"
...
def generate_detail_table(rows):
    """生成详细信息表格（折叠）"""
    lines = []
    lines.append("\n<details>\n")
...
def generate_legend():
    ...
    lines.append("| ✅ 需要 | 需要提前预约 |\n")
    lines.append("| ❌ 不需 | 无需预约，可直接前往 |\n")
    lines.append("| ⏱️ | 排队时长 |\n")
...
def generate_footer():
    ...
    lines.append("| contact | 联系方式 | 0512-12345678 |\n")
    lines.append("| reservation_needed | 是否需要预约 | 否 |\n")
    lines.append("| queue_time | 排队时间 | 5-20分钟 |\n")
    lines.append("| tags | 标签 | 生煎 |\n")
...
def main():
    ...
    lines.append(
        f"📊 共收录 **{len(rows)}** 家餐厅 ｜ 🕒 更新于·北京时间：{beijing_time.strftime('%Y-%m-%d %H:%M:%S')} CST\n\n")

    # 目录
    lines.append("## 📑 目录\n\n")
    lines.append("- [📊 数据统计](#-数据统计)\n")
    lines.append("- [📋 餐厅列表](#-餐厅列表)\n")
    lines.append("- [📖 详细信息](#-点击展开详细信息联系方式地址排队备注)\n")
    ...
    if stats['avg_queue'] > 0:
        print(f"  - 平均排队: {stats['avg_queue']:.0f} 分钟")
